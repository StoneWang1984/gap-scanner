"""rossway_daytrade_0.1C — 量价齐升 + 两段式退出 (OCO半仓止盈 + Trailing Stop 3%)

核心哲学: 量价齐升入场、OCO半仓止盈+trailing stop捕获大行情
- 扫描选股: 继承 stonewang_daytrade_1.3 scanner.py + strategy.py
- 入场确认: 3-bar pullback + 量价齐升 (确认bar放量≥均量1.5倍)
- Phase 1: OCO (止损+止盈2.5:1), 止盈卖出50%锁利
- Phase 2: 止盈后剩余50%转trailing stop 3%捕获大行情
- 不持仓过夜: EOD 15:50强制平仓
"""
...
from alpaca.trading.requests import (
    MarketOrderRequest, TrailingStopOrderRequest, LimitOrderRequest,
...
class Position:
    ...
    stop_price: float
    target_price: float
    sl_order_id: str = None       # stop loss order (Phase 1)
    tp_order_id: str = None       # take profit limit sell (Phase 1)
    trailing_order_id: str = None # trailing stop (Phase 2)
    phase: int = 1                # 1=OCO, 2=trailing
...
# ── Stop loss placement ──
def place_stop_loss(symbol, shares, stop_price):
    """Place stop loss order. Returns (order_id, None) on success, or (None, error_msg)."""
    if DRY_RUN:
        oid = f"DRY-SL-{uuid4().hex[:8]}"
        mock = MockOrder(id=oid, symbol=symbol, qty=shares, side="sell",
                         order_type="stop_loss", stop_price=stop_price, status="new")
        dry_run_orders[oid] = mock
        log(f"[DRY] STOP LOSS {symbol} {shares}sh @ ${stop_price:.2f}")
        return oid, None

    try:
        order = trading_client.submit_order(StopLossRequest(
            symbol=symbol, qty=shares, side=OrderSide.SELL,
            time_in_force=TimeInForce.DAY,
            stop_price=str(stop_price),
        ))
        order_id = str(order.id)
        log(f"STOP LOSS PLACED: {symbol} {shares}sh @ ${stop_price:.2f} -> {order_id}")
        return order_id, None
    except Exception as e:
        analysis = analyze_alpaca_rejection(e)
        log(f"{RED}STOP LOSS REJECTED {symbol}: {analysis['detail']}{RESET}")
        return None, str(e)


# ── Limit sell placement (take profit) ──
def place_limit_sell(symbol, shares, target_price):
    """Place limit sell order for take profit. Returns (order_id, None) on success, or (None, error_msg)."""
    if DRY_RUN:
        oid = f"DRY-TP-{uuid4().hex[:8]}"
        mock = MockOrder(id=oid, symbol=symbol, qty=shares, side="sell",
                         order_type="limit_sell", limit_price=target_price, status="new")
        dry_run_orders[oid] = mock
        log(f"[DRY] LIMIT SELL (TP) {symbol} {shares}sh @ ${target_price:.2f}")
        return oid, None
    ...
    try:
        ...
        order_id = str(order.id)
        log(f"LIMIT SELL (TP) PLACED: {symbol} {shares}sh @ ${target_price:.2f} -> {order_id}")
        return order_id, None
    except Exception as e:
        analysis = analyze_alpaca_rejection(e)
        log(f"{RED}LIMIT SELL REJECTED {symbol}: {analysis['detail']}{RESET}")
        return None, str(e)
...
def check_sl_fill(order_id, symbol):
    """Check if stop loss order has filled. Returns (filled, fill_price, fill_qty)."""
    if DRY_RUN:
        mo = dry_run_orders.get(order_id)
...
def check_tp_fill(order_id, symbol):
    """Check if take profit limit sell has filled. Returns (filled, fill_price, fill_qty)."""
    if DRY_RUN:
        mo = dry_run_orders.get(order_id)
        if not mo or mo.status != "filled":
            # Simulate: if current price >= target, fill
            price = _dry_run_get_price(symbol) or 0
            if price > 0 and mo and price >= mo.limit_price:
                mo.status = "filled"
                mo.filled_qty = mo.qty
                mo.filled_price = mo.limit_price
                return True, mo.limit_price, mo.qty
            return False, 0, 0
        return True, mo.filled_price, mo.filled_qty
            ...
            for ap in alpaca_positions:
                ...
                for ao in alpaca_orders:
                    if ao.symbol == sym and ao.side == OrderSide.SELL:
                        oid = str(ao.id)
                        order_type = str(ao.order_type).lower() if hasattr(ao, 'order_type') else ""
                        if "trailing_stop" in order_type or "trailing" in order_type:
                            trailing_id = oid
                            log(f"RECOVER: Found trailing stop {ao.id}")
                        elif "stop" in order_type:
                            sl_id = oid
                            log(f"RECOVER: Found stop loss {ao.id}")
                        elif "limit" in order_type:
                            tp_id = oid
                            log(f"RECOVER: Found limit sell {ao.id}")
                        else:
                            trailing_id = oid
    ...
    while True:
        ...
        if now_time >= force_close_time:
            ...
            for pos in positions[:]:
                log(f"EOD: Force closing {pos.symbol} {pos.shares}sh (phase {pos.phase})")
                if pos.phase == 1:
                    if pos.sl_order_id:
                        cancel_order(pos.sl_order_id)
                    if pos.tp_order_id:
                        cancel_order(pos.tp_order_id)
                elif pos.trailing_order_id:
        ...
        for pos in positions[:]:
            if pos.phase == 1:
                # Phase 1: Check stop loss and take profit fills
                # Check take profit first
                if pos.tp_order_id:
                    tp_filled, tp_price, tp_qty = check_tp_fill(pos.tp_order_id, pos.symbol)
                    if tp_filled and tp_qty > 0:
                        # Take profit filled → sell tp_qty, cancel stop loss, place trailing stop
                        tp_pnl = (tp_price - pos.entry_price) * tp_qty
                        log(f"{GREEN}TAKE PROFIT: {pos.symbol} {tp_qty}sh @ ${tp_price:.4f} P&L ${tp_pnl:.2f}{RESET}")
                        # Cancel stop loss
                        if pos.sl_order_id:
                            cancel_order(pos.sl_order_id)
                            pos.sl_order_id = None
                        pos.tp_order_id = None
                        # Remaining shares → trailing stop
                        remaining = pos.shares - tp_qty
                        if remaining > 0:
                            trail_id, trail_err = place_trailing_stop(pos.symbol, remaining)
                            if trail_err:
                                log(f"{YELLOW}Trailing stop failed for {pos.symbol}, force selling remaining{RESET}")
                                sold = force_sell_position(pos.symbol, remaining)
                                if sold > 0:
                                    trail_pnl = (sold * pos.entry_price * 0.98 - pos.entry_price * sold)
                                    trades_detail.append({
                                        "symbol": pos.symbol, "entry": pos.entry_price,
                                        "exit": pos.entry_price * 0.98, "shares": sold,
                                        "pnl": round(trail_pnl, 2), "reason": "trailing_failed",
                                        "trade_type": pos.trade_type,
                                    })
                                    daily_trades += 1
                                positions.remove(pos)
                                continue
                            pos.trailing_order_id = trail_id
                            pos.shares = remaining
                            pos.phase = 2
                            log(f"  {pos.symbol}: Phase 2 — {remaining}sh trailing {TRAILING_STOP_PCT*100:.1f}%")
                        else:
                            positions.remove(pos)
                        continue
                ...
                # Check stop loss
                if pos.sl_order_id:
                    sl_filled, sl_price, sl_qty = check_sl_fill(pos.sl_order_id, pos.symbol)
                    if sl_filled and sl_qty > 0:
                        sl_pnl = (sl_price - pos.entry_price) * sl_qty
                        log(f"{RED}STOP LOSS: {pos.symbol} {sl_qty}sh @ ${sl_price:.4f} P&L ${sl_pnl:.2f}{RESET}")
                        # Cancel take profit
                        if pos.tp_order_id:
                            cancel_order(pos.tp_order_id)
                            pos.tp_order_id = None
                        ...
                        trades_detail.append({
                            "symbol": pos.symbol, "entry": pos.entry_price,
                            "exit": sl_price, "shares": sl_qty,
                            "pnl": round(sl_pnl, 2), "reason": "stop_loss",
                            "trade_type": pos.trade_type,
                        })
                        daily_trades += 1
                        entry_checked.add(pos.symbol)
                        positions.remove(pos)
                        continue
                    elif sl_filled and sl_qty == 0:
                        # Stop loss canceled externally
                        log(f"{YELLOW}STOP LOSS CANCELED: {pos.symbol} — checking position{RESET}")
                        pos.sl_order_id = None

            elif pos.phase == 2:
                # Phase 2: Check trailing stop fill
        ...
        if entry_start <= now_time <= entry_end:
            ...
            if available_slots > 0:
                for c in candidates:
                    ...
                    tp_shares = max(1, int(actual_shares * TP_SELL_RATIO))

                    # Place stop loss (covers all shares)
                    sl_id, sl_err = place_stop_loss(sym, actual_shares, stop_price)
                    if sl_err:
                        log(f"{YELLOW}Stop loss failed for {sym}, force selling{RESET}")
                        sold = force_sell_position(sym, actual_shares)
                        if sold > 0:
                            pnl = (fill_price * 0.985 - fill_price) * sold
                            trades_detail.append({
                                "symbol": sym, "entry": fill_price,
                                "exit": fill_price * 0.985, "shares": sold,
                                "pnl": round(pnl, 2), "reason": "sl_failed",
                                "trade_type": "first",
                            })
                            daily_trades += 1
                        entry_checked.add(sym)
                    ...
                    # Place take profit limit sell (50% of shares)
                    tp_id, tp_err = place_limit_sell(sym, tp_shares, target_price)
                    if tp_err:
                        log(f"{YELLOW}Limit sell failed for {sym}, using trailing stop fallback{RESET}")
                        # Fallback: cancel stop loss, place trailing stop for all shares
                        if sl_id:
                            cancel_order(sl_id)
                        ...
                        pos = Position(
                            ...
                            stop_price=stop_price, target_price=target_price,
                            trailing_order_id=trail_id, phase=2,
                            entry_time=now_est, gap_pct=c["gap_pct"], trade_type="first",
                        )
                        positions.append(pos)
                        daily_trades += 1
                        available_slots -= 1
                        entry_checked.add(sym)
                        log(f"{GREEN}ENTERED: {sym} {actual_shares}sh @ ${fill_price:.4f} trailing {TRAILING_STOP_PCT*100:.1f}% (fallback) (gap {c['gap_pct']:.1%}){RESET}")
                        continue
                    ...
                    available_slots -= 1
                    entry_checked.add(sym)
                    log(f"{GREEN}ENTERED: {sym} {actual_shares}sh @ ${fill_price:.4f} stop=${stop_price:.2f} target=${target_price:.2f} (tp {tp_shares}sh) (gap {c['gap_pct']:.1%}){RESET}")
...
def main():
    ...
    log("=" * 60)
    log(f"rossway_daytrade_0.1C — {mode} Trading")
    log(f"Exit: Phase 1 OCO (stop=max(${STOP_LOSS_MIN_CENTS}, entry×{STOP_LOSS_PCT*100:.1f}%), target=stop×{REWARD_RISK_RATIO}, sell {TP_SELL_RATIO*100:.0f}% at TP)")
    log(f"      Phase 2 Trailing stop {TRAILING_STOP_PCT*100:.1f}% on remaining")
