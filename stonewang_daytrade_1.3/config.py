...
- RESCAN_TIMES = ["10:30", "11:30"] (mid-day re-scan for new candidates)
...
# ── Polling interval ──────────────────────────────────────────────────
POLL_INTERVAL = 3  # seconds (was 5; reduced to 3 after batch order cache optimization)

# ── OCO ladder sell parameters ──────────────────────────────────────
OCO_ENABLED = True           # True = OCO for T2+ (v1.2), False = polling fallback (v1.1)
OCO_STOP_BUFFER_PCT = 0.02  # Minimum OCO stop buffer (used as reference, actual buffer = trail_pct)
                             # T2 OCO stop = T1_fill × 0.98  (trail_pct[0]=2%)
                             # T3 OCO stop = T2_fill × 0.975 (trail_pct[1]=2.5%)
                             # Progressive: higher tiers get wider buffer for volatility
