"""Config — stonewang_daytrade_frik5bar_1.0: RTG + 5-Bar Filter + 10:25 Exit.
...
Strategy:
  ...
  - Entry: RTG signal (close > open_price + vol >= 1.5x prior) in 09:35-10:24
  - Exit: 10:25 EST market sell (time-based) or 3% hard stop loss
  - Position size: RVOL-weighted, max 8 concurrent

Backtest validation (3 months, 64 trading days):
  - Filter: bar1_bullish + first5chg>+2% → WinRate 76.3%, Avg +10.47%, Med +6.03%
  - Filter + gap<25% → WinRate 78.3%, Avg +7.36%
"""
...
# ── Version ──────────────────────────────────────────────────────────
VERSION = "stonewang_daytrade_frik5bar_1.0"
VERSION_SHORT = "frik5bar_1.0"
