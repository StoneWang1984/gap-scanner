"""stonewang_daytrade_frik5bar_1.0 — 5-Bar Filter + RTG + 10:25 Exit Live Trading.

Strategy:
  - Pre-market scan for gap-up stocks, rank by RVOL, select top 40
  - 5-Bar Filter: wait 5 min after open, then require:
      1. First bar bullish (close > open_price)
      2. First 5-min net change > +2%
      3. Gap < 25% (skip extreme gaps)
  - Entry: RTG signal (close > open + vol >= 1.5x prior) in 09:35-10:24
  - Exit: 10:25 EST market sell (time-based) or 3% hard stop loss
  - Position: RVOL-weighted, max 8 concurrent

Backtest validation (3 months): bar1_bullish + first5chg>+2% → 76.3% WR, +10.47% avg
"""
...
def check_frik5bar_filter(bars, open_price, gap_pct):
    ...
    bars: list of bar dicts from BarAccumulator (sorted by time).
...
def run_trading_day(target_date):
    log(f"Starting trading day: {target_date.date()} (filter=frik5bar, exit at {config.EXIT_TIME}, stop {config.STOP_LOSS_PCT:.0%})")
    try:
    ...
    while True:
        ...
        # Entry monitoring (09:35-10:24) — frik5bar entry starts at 09:35
        if entry_start_dt <= now < entry_end_dt and len(positions) < config.MAX_POSITIONS:
            ...
            for c in candidates:
                ...
                # ── FRIK5BAR: 5-bar filter → direct entry ──
                if getattr(config, "FRIK5BAR_ENABLED", False) and sym not in entry_checked:
                    passed, reason = check_frik5bar_filter(bars, open_price, c["gap_pct"])
                    if not passed:
                        log(f"FRIK5BAR skip {sym}: {reason}")
                        entry_checked.add(sym)
                        continue
                    # Filter passed → enter immediately at market
                    entry_price = bars[-1]["close"] if bars else open_price
                    ...
                    shares = int(slot / entry_price_actual)
                    if shares <= 0:
                        continue
                    ...
                    pos = Position(symbol=sym, shares=filled, entry_price=fill_price,
                                   ...
                                   gap_pct=c["gap_pct"], signal_type="frik5bar", highest=fill_price,
                                   rvol=rvol, stop_pct=config.STOP_LOSS_PCT)
                    positions.append(pos)
                    entry_checked.add(sym)
                    entry_count[sym] = entry_count.get(sym, 0) + 1
                    daily_trades += 1
                    live_bp -= fill_price * filled
                    log(f"ENTRY {sym} [frik5bar] {filled}sh @ ${fill_price:.4f} "
                        f"[RVOL={rvol:.1f}x stop={config.STOP_LOSS_PCT:.0%}]")
                    continue
...
def main():
    global _log_file
    _log_file = open(os.path.join(_ver_dir, "live_frik5bar.log"), "a")

    log(f"Using {config.DATA_FEED.upper()} data feed")
    log("=" * 60)
    log(f"stonewang frik5bar 1.0 — 5-Bar Filter + RTG + {config.EXIT_TIME} Exit")
    log(f"Entry window: {config.ENTRY_WINDOW_START}-{config.ENTRY_WINDOW_END}")
