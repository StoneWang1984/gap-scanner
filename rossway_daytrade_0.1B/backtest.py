"""Backtesting engine — rossway_daytrade_0.1B: 量价齐升 + trailing stop 2%."""
...
def run_backtest(end_date=None, n_days=30):
    ...
    print(f"[rossway 0.1B] Backtesting {len(trading_days)} days: {trading_days[0].date()} to {trading_days[-1].date()}")
    print(f"Trailing stop: {TRAILING_STOP_PCT*100:.1f}% | Volume ratio: {VOLUME_RATIO_MIN}x | Max positions: {config.MAX_POSITIONS}")
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            # Simulate trailing stop trade
            exit_price, exit_reason, exit0, highest = simulate_trailing_trade(
                entry_price, TRAILING_STOP_PCT, remaining, shares)
            ...
            print(f"  {symbol} {tag} entry=${entry_price:.4f} exit=${exit_price:.4f} ({exit_reason}), "
                  f"P&L=${pnl:,.2f} ({pnl_pct:.2%}), trail={TRAILING_STOP_PCT*100:.1f}%, high=${highest:.4f}")
            ...
            if exit_reason == "trailing_stop":
                entry_checked.add(symbol)  # Trailing stop → no re-entry for this symbol today
            else:
                # Force close → slot freed
                active_positions -= 1
    ...
    print(f"[rossway 0.1B] Backtest complete. Final equity: ${equity:,.2f}")
