"""Stone 0.4.7 Live Paper Trading — minimum pullback depth filter.
...
MIN_PULLBACK_PCT_047 = getattr(config, "MIN_PULLBACK_PCT", 0.015)
...
def scan_gaps():
    ...
    # 0.4.5: Filter out leveraged ETFs
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.7 Live Paper Trading")
    ...
    log(f"0.4.7: MIN_PULLBACK_PCT = {MIN_PULLBACK_PCT_047:.0%} (skip shallow pullbacks)")
    log("=" * 60)
    ...
    while True:
        ...
        if now_time < cutoff_time and daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
            for cand in candidates:
                ...
                pullback_depth = (cand["open_price"] - entry_price) / cand["open_price"] if cand["open_price"] > 0 else 0
                if pullback_depth < MIN_PULLBACK_PCT_047:
                    log(f"  {symbol}: pullback {pullback_depth:.2%} < {MIN_PULLBACK_PCT_047:.0%} minimum, skipping")
                    entry_checked.add(symbol)
                    continue
        ...
        # ── Check re-entry ──
        # 0.4.5: No re-entries after reentry_cutoff_time
