"""Stone 0.4.15 Live Paper Trading — based on 0.4.14, tuned safety params.

Changes over 0.4.14:
- MAX_DAILY_LOSS_PCT: 5% → 8% (avoid premature circuit breaker)
- REENTRY_MIN_PULLBACK: 3% → 0% (removed; was filtering profitable re-entries)
"""
...
_spec = importlib.util.spec_from_file_location("config", os.path.join(_ver_dir, "config_stone_0.4.15.py"))
...
_LOG_FILE = os.path.join(_ver_dir, "live_0415.log")
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.15 Live Paper Trading — Auto Scheduler")
    ...
    log(f"0.4.15: Based on 0.4.14, daily loss 8% circuit breaker, no re-entry min pullback")
    log("=" * 60)
    ...
    while True:
        ...
        if now_time < reentry_cutoff_time and daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
            ...
            for symbol in exited_symbols:
                ...
                # 0.4.15: Minimum pullback from peak for re-entry
                ...
                # 0.4.15: Daily loss circuit breaker
                ...
                # 0.4.15: Cap re-entry stop loss at max percentage
