"""Config — Stone 0.4.11: entry below open + time limit exit.

Changes over 0.4.10:
- New rule: if first trade entry price >= open price, skip the trade
- Fixed entry confirmation: running-minimum approach for true pullback bottom
- Time limit: if no target hit in 40 min, sell all when price >= entry price
"""
...
# ── Re-entry trade — 0.4.10+: redesigned ─────────────────────────────
...
REENTRY_PROFIT_RETRACEMENT = 1.50       # legacy 150% target (unused by v2)
REENTRY_PROFIT_RETRACEMENT_1 = 0.75     # v2 tier-1: sell 1/2 at 75% retracement
REENTRY_SELL_RATIO = 1/3                # legacy (unused by v2)
REENTRY_SELL_RATIO_1 = 0.5             # v2 tier-1: sell 1/2 of position
REENTRY_TRAILING_PCT = 0.05             # legacy (unused by v2)
