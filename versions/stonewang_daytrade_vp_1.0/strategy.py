...
def evaluate_trade_vp(
    ...
    """VP backtest: exit on drop or time limit, whichever comes first.

    - drop_exit: bar low <= entry_price * (1 - drop_pct) → exit at cutoff
    - time_limit: bi >= time_limit_bars → exit at bar close
    - force_close: end of bars without trigger

    Slippage: SLIPPAGE_FORCE_CLOSE_PCT applied to exit price (conservative).
    Note: 1-min bar is a coarse approximation of trade-print derivatives.
    Live uses real-time WS trades; backtest uses 1-min bar low/close.
    """
    if drop_pct is None:
    ...
    # Convert seconds to bars (1-min bars)
    time_limit_bars = max(1, time_limit_sec // 60)

    slippage = getattr(config, "SLIPPAGE_FORCE_CLOSE_PCT", 0)
    cutoff = round(entry_price * (1 - drop_pct), 4)

    import pandas as pd

    def _make_result(reason: str, exit_price: float, bi: int) -> TradeResult:
        pnl = (exit_price - entry_price) * shares
        pnl_pct = pnl / (entry_price * shares) if entry_price > 0 else 0
        _date = ""
        if 0 <= bi < len(bars_after_entry):
            bar = bars_after_entry[bi]
            _date = str(bar.get("timestamp", pd.Timestamp.now()).date())
        return TradeResult(
            symbol=symbol, date=_date,
            ...
            pnl=round(pnl, 2), pnl_pct=round(pnl_pct, 4), exit_reason=reason,
            open_price=open_price, sell_target=0,
            stop_price=round(cutoff, 2),
            partial_sells=[], trailing_high=0.0,
            trailing_exit_price=exit_price, atr=0.0,
            exit_bar_idx=bi, position_size=entry_price * shares,
            trade_type="first",
        )

    for bi, bar in enumerate(bars_after_entry):
        # Drop exit: bar low breached cutoff
        if bar["low"] <= cutoff:
            exit_price = cutoff
            if slippage > 0:
                exit_price = round(exit_price * (1 - slippage), 2)
            return _make_result("drop_exit", exit_price, bi)

        # Time limit: held long enough without drop
    ...
    # No trigger hit — force close at end of bar series
    if force_close_price is not None:
        exit_price = force_close_price
    elif bars_after_entry:
