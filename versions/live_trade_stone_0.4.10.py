"""Stone 0.4.10 Live Paper Trading — leveraged ETF filter + re-entry v2 strategy.

Changes over 0.4.5:
- Re-entry v2 strategy:
  - Half position size for re-entries (REENTRY_POSITION_RATIO = 0.5)
  - ATR-based stop for re-entries (1.5 * ATR, floored at 4%)
  - Tier-1 / tier-2 profit targets (75% retracement then trailing)
  - Breakeven stop after tier-1 partial fill
  - Time stop: exit if no tier-1 target in 6 bars
  - Cutoff time 12:30 (was 13:00)
- LivePosition new fields: reached_target1, sold_partial1_shares,
  breakeven_active, reentry_bar_count, atr
- Price range $1-$20 (config handles this)
"""
...
_spec = importlib.util.spec_from_file_location("config", os.path.join(_ver_dir, "config_stone_0.4.10.py"))
...
def run_live():
    ...
    log(f"Re-entry v2: half-pos, ATR stop, tier-1/tier-2 targets, breakeven, time stop")
    log("=" * 60)
