"""Stone 0.4.12 Live Paper Trading — TARGET_150 full exit + re-entry two-tier % targets.
...
_spec = importlib.util.spec_from_file_location("config", os.path.join(_ver_dir, "config_stone_0.4.12.py"))
...
_LOG_FILE = os.path.join(_ver_dir, "live_0412.log")
...
class LivePosition:
    ...
    reached_target2: bool = False
    sold_partial1_shares: int = 0
    reentry_bar_count: int = 0
    reentry_time_limit_active: bool = False
    atr: float = 0.0
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.12 Live Paper Trading — Auto Scheduler")
    ...
    log("Re-entry 0.4.12: 50%@+3% + 50%@+5% | ATR stop | Time limit 1hr")
    log("0.4.12: TARGET_150 full exit | Skip if entry >= open | Fixed running-min confirmation")
    log("=" * 60)
...
def run_trading_day(force_close_time: dt.time, force_close_str: str,
    ...
    while True:
        ...
        for pos in positions[:]:
            ...
            if pos.trade_type == "first":
                ...
                if not pos.reached_150 and pos.highest >= pos.target_150:
                    ...
                    # 0.4.12: sell ALL remaining shares at 150% target
                    total_sell = pos.remaining_shares
                    if total_sell > 0:
                        sell_price = round(pos.target_150 * (1 - TARGET_LIMIT_BUFFER), 2)
                        log(f"150% TARGET (FULL EXIT): {pos.symbol} selling {total_sell} @ ${sell_price:.4f}")
                        ...
                        pos.sold_150_shares = total_sell
                        pos.remaining_shares = 0
                    # Trade complete — no trailing after 150%
                    record_trade(pos, pos.target_150, "target_150_full")
                    positions.remove(pos)
                    continue
            ...
            # ── Re-entry 0.4.12: two-tier percentage targets ──
            elif pos.trade_type == "reentry":
                need_replace_protective = False
                pos.reentry_bar_count += 1
                reentry_pct1 = getattr(config, "REENTRY_PROFIT_PCT_1", 0.03)
                reentry_pct2 = getattr(config, "REENTRY_PROFIT_PCT_2", 0.05)
                reentry_tl = getattr(config, "REENTRY_TIME_LIMIT_BARS", 12)
                tier1_price = round(pos.entry_price * (1 + reentry_pct1), 2)
                tier2_price = round(pos.entry_price * (1 + reentry_pct2), 2)

                # Time limit: if no tier-1 within reentry_tl bars, sell at breakeven or better
                if reentry_tl > 0 and not pos.reached_target1 and pos.reentry_bar_count >= reentry_tl:
                    pos.reentry_time_limit_active = True
                if pos.reentry_time_limit_active and cur_price >= pos.entry_price and pos.remaining_shares > 0:
                    log(f"RE-ENTRY TIME LIMIT EXIT: {pos.symbol} @ ${cur_price:.4f}")
                    events_log.append(f"{now_est.strftime('%H:%M:%S')} RE-ENTRY TIME LIMIT {pos.symbol} @ ${cur_price:.4f}")
                    add_chart_event(pos.symbol, "sell", cur_price, f"TIME LIMIT {pos.remaining_shares}sh")
                    if pos.protective_order_id:
                        cancel_order(pos.protective_order_id)
                        pos.protective_order_id = None
                    sold = force_sell_position(pos.symbol, pos.remaining_shares)
                    pos.remaining_shares = 0
                    record_trade(pos, cur_price, "reentry_time_limit_exit")
                    positions.remove(pos)
                    continue

                # Tier-1: sell 50% at +3%
                if not pos.reached_target1 and pos.highest >= tier1_price:
                    pos.reached_target1 = True
                    sell_ratio_1 = getattr(config, "REENTRY_SELL_RATIO_1", 0.5)
                    n = int(pos.remaining_shares * sell_ratio_1)
                    if n > 0:
                        sell_price = round(tier1_price * (1 - TARGET_LIMIT_BUFFER), 2)
                        log(f"RE-ENTRY 3% TARGET: {pos.symbol} selling {n} @ ${sell_price:.4f}")
                        events_log.append(f"{now_est.strftime('%H:%M:%S')} RE-ENTRY 3% {pos.symbol} sell {n} @ ${sell_price:.4f}")
                        add_chart_event(pos.symbol, "sell", sell_price, f"3% TARGET {n}sh")
                        if pos.protective_order_id:
                            cancel_order(pos.protective_order_id)
                            pos.protective_order_id = None
                ...
                # Tier-2: sell all remaining at +5%
                if pos.reached_target1 and not pos.reached_target2 and pos.highest >= tier2_price:
                    pos.reached_target2 = True
                    n = pos.remaining_shares
                    if n > 0:
                        sell_price = round(tier2_price * (1 - TARGET_LIMIT_BUFFER), 2)
                        log(f"RE-ENTRY 5% TARGET: {pos.symbol} selling {n} @ ${sell_price:.4f}")
                        events_log.append(f"{now_est.strftime('%H:%M:%S')} RE-ENTRY 5% {pos.symbol} sell {n} @ ${sell_price:.4f}")
                        add_chart_event(pos.symbol, "sell", sell_price, f"5% TARGET {n}sh")
                        if pos.protective_order_id:
                            cancel_order(pos.protective_order_id)
                            pos.protective_order_id = None
                        place_sell_limit(pos.symbol, n, sell_price)
                        pos.remaining_shares = 0
                        record_trade(pos, tier2_price, "reentry_tier2")
                        positions.remove(pos)
                        continue
        ...
        # ── Check re-entry ──
        # 0.4.10: Re-entry v2 with half position, ATR stop, tier targets, time stop
        if now_time < reentry_cutoff_time and daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
            ...
            for symbol in exited_symbols:
                ...
                # 0.4.12: ATR-based stop for re-entry
                atr = calc_atr(bars_5m, period=14) if len(bars_5m) >= 2 else get_prev_day_atr(symbol)
                if atr > 0:
                    stop = round(entry_price - getattr(config, "REENTRY_STOP_ATR_MULT", 1.5) * atr, 2)
                    fallback = round(entry_price * (1 - getattr(config, "REENTRY_STOP_PCT_FALLBACK", 0.04)), 2)
                    stop = max(stop, fallback)
                else:
                    stop = round(entry_price * (1 - config.REENTRY_STOP_PCT), 2)

                # 0.4.12: Tier-1 target = +3%, Tier-2 target = +5%
                reentry_pct1 = getattr(config, "REENTRY_PROFIT_PCT_1", 0.03)
                reentry_pct2 = getattr(config, "REENTRY_PROFIT_PCT_2", 0.05)
                tier1_price = round(entry_price * (1 + reentry_pct1), 2)
                tier2_price = round(entry_price * (1 + reentry_pct2), 2)

                # Half position size for re-entry
                pos_size = min(calc_position_size(capital), config.MAX_POSITION_SIZE)
                ...
                if order:
                    pos = LivePosition(
                        ...
                        reentry_target=tier1_price, entry_time=now_est,
                        atr=atr,
                    )
                    positions.append(pos)
                    place_protective_stop(pos)
                    ...
                    log(f"RE-ENTERED {symbol}: entry=${entry_price:.4f}, limit=${limit_price:.4f}, "
                        f"stop=${stop:.4f}, t1=${tier1_price:.4f}, t2=${tier2_price:.4f}, shares={shares}, atr=${atr:.4f}")
                    events_log.append(f"{now_est.strftime('%H:%M:%S')} RE-ENTERED {symbol} @ ${entry_price:.4f} (0.4.12)")
        ...
        if poll_count % 4 == 0 and positions:
            for pos in positions:
                ...
                if pos.trade_type == "reentry":
                    tier_info = f", t1={'Y' if pos.reached_target1 else 'N'}, t2={'Y' if pos.reached_target2 else 'N'}, bars={pos.reentry_bar_count}"
