"""Stone 0.4.11 Live Paper Trading — entry below open + fixed confirmation.

Changes over 0.4.10:
- Skip first trade if entry price >= open price (no chasing above open)
- Fixed entry confirmation: running-minimum approach finds true pullback bottom
"""
...
_spec = importlib.util.spec_from_file_location("config", os.path.join(_ver_dir, "config_stone_0.4.11.py"))
...
_LOG_FILE = os.path.join(_ver_dir, "live_0411.log")
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.11 Live Paper Trading — Auto Scheduler")
    ...
    log(f"0.4.11: Skip if entry >= open | Fixed running-min confirmation")
    log("=" * 60)
