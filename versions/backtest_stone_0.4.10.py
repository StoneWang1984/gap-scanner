"""Backtesting engine — Stone 0.4.10: price $1-$20 + improved re-entry strategy.

Changes over 0.4.9:
- Price range: $1-$20 (exclude penny stocks and $20-30 range)
- Re-entry redesigned:
  - Half position size (50% of first trade)
  - ATR-based stop loss (1.5× ATR, fallback 4%)
  - Dual-tier profit: sell 1/2 @ 75% retracement, then 3% trailing
  - Breakeven stop after first tier sell
  - Time stop: exit if no tier-1 target in 6 bars (30 min)
  - Cutoff time: 12:30 (was 13:00)
"""
...
def evaluate_reentry_trade_v2(
    ...
    for bi, bar in enumerate(bars_after_entry):
        ...
        # Stop loss (breakeven after tier-1)
...
def run_backtest_0410(end_date=None, n_days=config.BACKTEST_DAYS) -> tuple[list[TradeResult], float]:
    ...
    print(f"[Stone 0.4.10] Backtesting {len(trading_days)} trading days: "
    ...
    print(f"Scanner: PRICE \${config.PRICE_MIN:.0f}-\${config.PRICE_MAX:.0f} | MIN_VOL={config.MIN_VOLUME:,} | MIN_$VOL=${config.MIN_DOLLAR_VOLUME:,}")
    reentry_pos = getattr(config, "REENTRY_POSITION_RATIO", 0.5)
    reentry_atr = getattr(config, "REENTRY_STOP_ATR_MULT", 1.5)
    reentry_fallback = getattr(config, "REENTRY_STOP_PCT_FALLBACK", 0.04)
    reentry_trail2 = getattr(config, "REENTRY_TRAILING_PCT_2", 0.03)
    reentry_maxbars = getattr(config, "REENTRY_MAX_BARS_BEFORE_TARGET", 6)
    print(f"Re-entry v2: {reentry_pos:.0%} position | ATR×{reentry_atr} stop ({reentry_fallback:.0%} fallback) | "
          f"1/2@75% → 3% trail | Breakeven ON | Time stop {reentry_maxbars} bars | Cutoff {reentry_cutoff}")
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            while daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
                ...
                # Use ATR from first trade for re-entry stop
                ...
                print(f"  {symbol} {type_tag} entry=${reentry_result.entry_price:.4f} exit=${reentry_result.exit_price:.4f} "
                      ...
                      f", prev_high=${prev_high:.4f}, target=${reentry_result.sell_target:.4f}{re_extra}")
                ...
                if reentry_result.exit_reason in ("reentry_force_close", "reentry_early_exit"):
                    break

    print(f"\n{'='*60}")
    print(f"[Stone 0.4.10] Backtest complete.")
