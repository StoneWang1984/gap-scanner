"""Backtesting engine — Stone 0.4.5: re-entry cutoff + wider trailing + leveraged ETF filter.

Changes over 0.4.4:
- Re-entry time cutoff: no re-entries after 13:00 EST (avoids force-close losses)
- Trailing stop at 75% tier: 3% → 5% (more room for price movement)
- Leveraged ETF exclusion: skip symbols with leveraged suffixes (2x/3x ETFs)
"""
...
# ── Slippage parameters (unchanged from 0.4.4) ────────────────────────
...
# ── 0.4.5: Leveraged ETF detection ───────────────────────────────────
# Common patterns: suffix U/L (bull/bear), contains 2X/3X, etc.
_LEV_PATTERN = re.compile(r'(2X|3X|BULL|BEAR)$', re.IGNORECASE)
_LEV_SUFFIXES = ("U", "L")  # single-letter bull/bear suffixes
...
def is_leveraged_etf(symbol: str) -> bool:
    ...
    if _LEV_PATTERN.search(symbol):
        return True
    # Single-char U/L suffix only if symbol length > 3 (avoid false positives like "MU")
    if len(symbol) > 3 and symbol[-1] in _LEV_SUFFIXES:
        return True
    # Known leveraged ETF families
...
def run_backtest_045(end_date=None, n_days=config.BACKTEST_DAYS) -> tuple[list[TradeResult], float]:
    ...
    print(f"[Stone 0.4.5] Backtesting {len(trading_days)} trading days: "
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            while daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
                ...
                # Find the timestamp of the first bar after exit
    ...
    print(f"\n{'='*60}")
    print(f"[Stone 0.4.5] Backtest complete.")
