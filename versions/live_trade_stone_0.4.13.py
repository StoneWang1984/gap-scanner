"""Stone 0.4.13 Live Paper Trading — based on 0.4.11, remove re-entry time limit.

Changes over 0.4.11:
- Remove re-entry time stop (REENTRY_MAX_BARS_BEFORE_TARGET = 0)
  A/B testing showed re-entry time limits reduce performance
"""
...
_spec = importlib.util.spec_from_file_location("config", os.path.join(_ver_dir, "config_stone_0.4.13.py"))
...
_LOG_FILE = os.path.join(_ver_dir, "live_0413.log")
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.13 Live Paper Trading — Auto Scheduler")
    ...
    log(f"0.4.13: Based on 0.4.11, removed re-entry time limit")
    log("=" * 60)
