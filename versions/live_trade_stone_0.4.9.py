"""Stone 0.4.9 Live Paper Trading — leveraged ETF filter + re-entry v2 strategy.

Changes over 0.4.5:
- Re-entry v2: half position size, ATR-based stop, tier-1 target with breakeven
- Time stop: exit re-entry if no tier-1 target in max_bars
- Breakeven stop after tier-1 partial sell
- Trailing stop after tier-1 (3% by default)
- Re-entry cutoff: 12:30 EST (was 13:00)
"""
...
def scan_gaps():
    ...
    # 0.4.9: Filter out leveraged ETFs
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.9 Live Paper Trading")
    ...
    log(f"Re-entry cutoff: {REENTRY_CUTOFF} EST | Leveraged ETF filter: ON | Re-entry v2: ON")
    log("=" * 60)
    ...
    while True:
        ...
        for pos in positions[:]:
            ...
            # ── Re-entry v2 profit targets ──
                ...
                # Time stop: exit if no tier-1 target in max_bars
                if not pos.reached_target1 and pos.reentry_bar_count >= max_bars:
                    log(f"RE-ENTRY TIME STOP: {pos.symbol} (no target in {max_bars} bars)")
                    events_log.append(f"{now_est.strftime('%H:%M:%S')} RE-ENTRY TIME STOP {pos.symbol}")
                    if pos.protective_order_id:
                        cancel_order(pos.protective_order_id)
                        pos.protective_order_id = None
                    sold = force_sell_position(pos.symbol, pos.remaining_shares)
                ...
                if need_replace_protective and pos.remaining_shares > 0:
                    # After tier-1, use trailing stop
        ...
        # ── Check re-entry ──
        # 0.4.9: No re-entries after reentry_cutoff_time (12:30 EST)
            ...
            for symbol in exited_symbols:
                ...
                if order:
                    ...
                    log(f"RE-ENTERED {symbol}: entry=${entry_price:.4f}, limit=${limit_price:.4f}, "
                        f"stop=${stop:.4f}, target=${target:.4f}, prev_high=${prev_high:.4f}, "
                        f"shares={shares}, atr={atr:.4f}, v2=tier1+breakeven+trail")
        ...
        if poll_count % 4 == 0 and positions:
            for pos in positions:
                ...
                if pos.trade_type == "reentry":
                    extra = f", t1={'Y' if pos.reached_target1 else 'N'}, be={'Y' if pos.breakeven_active else 'N'}, bars={pos.reentry_bar_count}, atr={pos.atr:.4f}"
                log(f"  {pos.symbol}({pos.trade_type}): {pos.remaining_shares} shares, "
                    f"entry=${pos.entry_price:.4f} cur=${cur:.4f} pnl=${pnl:.2f}{protective}{extra}")
