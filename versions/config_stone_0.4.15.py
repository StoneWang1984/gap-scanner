"""Config — Stone 0.4.15: based on 0.4.14, tuned safety parameters.

Changes over 0.4.14:
- MAX_DAILY_LOSS_PCT: 5% → 8% (avoid premature circuit breaker on single stop)
...
# ── Re-entry trade — 0.4.15: retracement + trailing, no time limit ───
...
REENTRY_MAX_BARS_BEFORE_TARGET = 0      # no time stop (0.4.13: removed)

# 0.4.15: Removed REENTRY_MIN_PULLBACK (was filtering profitable re-entries)
REENTRY_MIN_PULLBACK = 0

PULLBACK_STOP_THRESHOLD = 0.15

# ── 0.4.15: Daily loss circuit breaker ───────────────────────────────
# 5% was too tight — single stop_loss on compounded position could trigger it
