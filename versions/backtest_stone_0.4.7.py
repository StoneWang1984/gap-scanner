"""Backtesting engine — Stone 0.4.7: minimum pullback depth filter.

Changes over 0.4.5:
- MIN_PULLBACK_PCT = 0.03: skip entries where pullback from open < 3%
  Shallow pullbacks produce clustered profit targets and thin margins after slippage.
"""
...
def run_backtest_047(end_date=None, n_days=config.BACKTEST_DAYS) -> tuple[list[TradeResult], float]:
    ...
    min_pullback = getattr(config, "MIN_PULLBACK_PCT", 0.0)

    print(f"[Stone 0.4.7] Backtesting {len(trading_days)} trading days: "
    ...
    print(f"0.4.7 new: MIN_PULLBACK_PCT = {min_pullback:.0%} (skip shallow pullbacks)")
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            # 0.4.7: Minimum pullback depth filter
            pullback_depth = (open_price - pullback) / open_price if open_price > 0 else 0
            if pullback_depth < min_pullback:
                print(f"  {symbol}: pullback {pullback_depth:.2%} < {min_pullback:.0%} minimum, skipping")
                skipped_pullback += 1
                continue
            ...
            while daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
                ...
                if reentry_result.partial_sell_shares > 0:
                    re_extra += f", 1/3@${reentry_result.partial_sell_shares:.4f}"
    ...
    print(f"\n{'='*60}")
    print(f"[Stone 0.4.7] Backtest complete.")
    print(f"Final equity: ${equity:,.2f} | Total slippage cost: ${total_slippage:,.2f}")
    print(f"Total trades: {len(all_trades)}")
    print(f"0.4.7 filter: skipped {skipped_pullback} (pullback < {min_pullback:.0%})")
