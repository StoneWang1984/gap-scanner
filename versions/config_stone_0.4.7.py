"""Config — Stone 0.4.7: minimum pullback depth filter.

Changes over 0.4.5:
- MIN_PULLBACK_PCT = 0.015: skip entries where pullback from open < 1.5%
  Shallow pullbacks produce clustered targets and thin margins after slippage.
  Also filters negative pullback (entry > open) which is never a true pullback.
  3% was too aggressive (filtered good trades like ABSI +$119 at 1.58% pullback).
"""
...
# ── Stone 0.4.7: Minimum pullback depth ──────────────────────────────
MIN_PULLBACK_PCT = 0.015   # skip if (open - entry) / open < 1.5%
