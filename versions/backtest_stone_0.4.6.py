"""Backtesting engine — Stone 0.4.6: sub-dollar filter + gap cap.

Changes over 0.4.5:
- PRICE_MIN raised from $0.25 to $1.00 (exclude sub-dollar stocks / pump-and-dump)
- GAP_MAX = 0.30 (exclude extreme gaps likely pump-and-dump, e.g. UZX +40.8%)
"""
...
# ── 0.4.6: Leveraged ETF detection (unchanged from 0.4.5) ────────────
...
# ── 0.4.6: Gap size cap ─────────────────────────────────────────────
GAP_MAX = getattr(config, "GAP_MAX", 0.30)
...
def run_backtest_046(end_date=None, n_days=config.BACKTEST_DAYS) -> tuple[list[TradeResult], float]:
    ...
    gap_max = getattr(config, "GAP_MAX", 0.30)
    price_min = getattr(config, "PRICE_MIN", 1.00)

    print(f"[Stone 0.4.6] Backtesting {len(trading_days)} trading days: "
    ...
    print(f"0.4.6 new: PRICE_MIN=${price_min:.2f} | GAP_MAX={gap_max:.0%}")
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            # 0.4.5: Skip leveraged ETFs
            if is_leveraged_etf(symbol):
                print(f"  {symbol}: leveraged ETF, skipping")
                continue

            # 0.4.6: Gap size cap
            gap_pct = (open_price / row.get("prev_close", open_price)) - 1.0 if row.get("prev_close", 0) > 0 else 0
            # Use gap_pct from the row if available
            if "gap_pct" in row:
                gap_pct = row["gap_pct"]
            if gap_pct > gap_max:
                print(f"  {symbol}: gap {gap_pct:.1%} > {gap_max:.0%} cap, skipping")
                skipped_gap_cap += 1
                continue

            # 0.4.6: Sub-dollar stock filter
            if open_price < price_min:
                print(f"  {symbol}: open ${open_price:.4f} < ${price_min:.2f}, skipping")
                skipped_price_min += 1
                continue
            ...
            while daily_trades < config.MAX_DAILY_TRADES and not daily_stopped:
                ...
                # 0.4.5: Check re-entry time cutoff
    ...
    print(f"\n{'='*60}")
    print(f"[Stone 0.4.6] Backtest complete.")
    print(f"Final equity: ${equity:,.2f} | Total slippage cost: ${total_slippage:,.2f}")
    print(f"Total trades: {len(all_trades)}")
    print(f"0.4.6 filters: skipped {skipped_gap_cap} (gap cap) + {skipped_price_min} (sub-dollar)")
