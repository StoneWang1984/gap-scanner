"""Stone 0.4.6 Live Paper Trading — sub-dollar filter + gap cap + leveraged ETF filter + re-entry cutoff.

Changes over 0.4.5:
- PRICE_MIN raised from $0.25 to $1.00 (exclude sub-dollar stocks)
- GAP_MAX = 0.30 (exclude extreme gaps likely pump-and-dump)
"""
...
PRICE_MIN_046 = getattr(config, "PRICE_MIN", 1.00)
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for symbol in batch:
            try:
                ...
                # 0.4.6: Sub-dollar filter
                if open_price < PRICE_MIN_046:
                    continue
                if not (PRICE_MIN_046 <= open_price <= config.PRICE_MAX):
                    continue
...
def run_live():
    log("=" * 60)
    log("Stone 0.4.6 Live Paper Trading")
    ...
    log(f"0.4.6 new: PRICE_MIN=${PRICE_MIN_046:.2f} | GAP_MAX={GAP_MAX:.0%}")
    log("=" * 60)
    ...
    while True:
        ...
        # ── Check re-entry ──
        # No re-entries after reentry_cutoff_time
