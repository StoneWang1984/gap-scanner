"""Backtesting engine — Stone 1.3.2ts: phased trailing stop (10%→3%)."""
...
def run_backtest(end_date=None, n_days=config.BACKTEST_DAYS) -> list[TradeResult]:
    ...
    print(f"[Stone 1.3.2ts] Backtesting {len(trading_days)} trading days: {trading_days[0].date()} to {trading_days[-1].date()}")
    ...
    for date in trading_days:
        ...
        for _, row in candidates.iterrows():
            ...
            # ── Phased trailing stop exit (v1.3.2ts) ──
    ...
    print(f"\n{'='*60}")
    print(f"[Stone 1.3.2ts] Backtest complete. Final equity: ${equity:,.2f}")
