"""Backtesting engine — stonewang_daytrade_keep_raising_1.0: Morning RTG + Afternoon Keep Raising.

Key features vs rtg_2.0:
  1. Keep Raising mode (10:30-15:45): scan for small-amplitude steady uptrend
  2. KR score = up_bar_ratio × consistency × amplitude_quality
  3. KR exit: 3 consec down bars / drop 0.5% from high / max hold 120min
  4. Same morning RTG + vol_surge as rtg_2.0
  5. Progressive trailing + daily profit protection
"""
...
# ── Keep Raising scoring (same as live_trade.py) ──────────────────────

def score_keep_raising(bars):
    """Score a window of bars for keep-raising pattern.

    Returns (score, metrics_dict). score=0 means doesn't qualify.
    """
    if len(bars) < 5:
        return 0.0, {}

    # Total gain from window
    first_close = bars[0]["close"]
    last_close = bars[-1]["close"]
    if first_close <= 0:
        return 0.0, {}
    total_gain_pct = (last_close - first_close) / first_close
    ...
    # up_ratio: fraction of bars where close > open
    up_count = sum(1 for b in bars if b["close"] > b["open"])
    up_ratio = up_count / len(bars)
    if up_ratio < getattr(config, "KR_MIN_UP_BARS_RATIO", 0.60):
        return 0.0, {}

    # consistency: fraction of bars where close > prev close
    consistent_count = 0
    for i in range(1, len(bars)):
        if bars[i]["close"] > bars[i - 1]["close"]:
            consistent_count += 1
    consistency = consistent_count / (len(bars) - 1) if len(bars) > 1 else 0
    ...
    total_move = abs(last_close - first_close)
    if total_move <= 0:
        return 0.0, {}
    amplitude_ratio = avg_range / total_move
...
class OpenPosition:
    ...
    def _get_progressive_trail(self, profit_pct):
        # Use KR progressive tiers for keep_raising positions
        if self.signal_type == "keep_raising":
            tiers = getattr(config, "KR_PROGRESSIVE_TRAIL_TIERS", [])
        else:
    ...
    def evaluate_bar(self, bar, bar_idx):
        ...
        if self.signal_type == "keep_raising":
            # We need bars list context for KR exits — handled in simulation loop
            # Standard stop and trail still apply here
            pass
    ...
    def evaluate_kr_exit(self, bars, bar_idx):
        """Keep Raising specific exit checks. Returns (should_exit, reason)."""
        if self.closed or self.signal_type != "keep_raising":
            return False, ""

        # Get recent bars up to current bar_idx
        ...
        cur_price = bars[bar_idx]["close"]
        if self.highest > 0 and cur_price < self.highest * (1 - drop_pct):
            return True, "kr_drop_from_high"

        # Trigger 3: Max hold time
        max_hold = getattr(config, "KR_EXIT_MAX_HOLD_MINUTES", 120) * 60
        # Estimate entry time from bar index
        entry_minutes = (bar_idx - self.entry_bar_idx)
        if entry_minutes >= max_hold / 60:
...
def save_backtest_charts(chart_entries, filepath="versions/chart_data_kr1.json"):
...
def run_backtest(end_date=None, n_days=None):
    ...
    print(f"[keep_raising_1.0] Backtesting {len(trading_days)} trading days: "
    ...
    if kr_enabled:
        print(f"  Keep Raising: {config.KEEP_RAISING_START}-{config.KEEP_RAISING_ENTRY_END} EST, "
              f"lookback={config.KR_LOOKBACK_MINUTES}min, "
              f"up>={config.KR_MIN_UP_BARS_RATIO:.0%}, cons>={config.KR_MIN_CONSISTENCY_SCORE:.0%}, "
              f"amp<={config.KR_MAX_AMPLITUDE_RATIO:.0%}")
    ...
    for date in trading_days:
        ...
        # ── Pre-fetch 1-min bars for ALL gap candidates ──
        for _, row in (candidates.iterrows() if not candidates.empty else []):
            symbol = row["symbol"]
        ...
        # Simulate bar by bar
        kr_last_scan_bar_idx = -999  # Track when we last scanned for KR entries
        ...
        for ts in sorted_times:
            ...
            for pos in open_positions:
                ...
                # Keep Raising specific exit checks
                if not pos.closed and pos.signal_type == "keep_raising":
                    should_exit_kr, kr_reason = pos.evaluate_kr_exit(bars, bar_idx)
                    if should_exit_kr:
                        exit_slippage = getattr(config, "SLIPPAGE_EXIT_PCT", 0.0)
                        pos.exit_price = round(bar["close"] * (1 - exit_slippage), 4)
                        pos.exit_reason = kr_reason
                        ...
                        if price_declining:
                            ...
                            for bi, b in enumerate(bars):
                                if b["timestamp"] == ts:
                                    bar_idx_fc = bi
                                    break
                            p.force_close(cur_price, bar_idx_fc, "profit_protect")
            ...
            if max_daily_loss > 0 and realized_so_far <= -max_daily_loss:
                for p in open_positions:
                    if not p.closed:
                        ...
                        if "close" in bar:
                            bar_idx_fc = 0
                            for bi, b in enumerate(cached_bars.get(p.symbol, [])):
                                if b["timestamp"] == ts:
                                    bar_idx_fc = bi
                                    break
                            p.force_close(bar["close"], bar_idx_fc, "daily_loss_limit")
                            closed_trades.append(p)
                print(f"  Daily loss ${realized_so_far:,.2f} exceeded limit, stopping")
                break

            # ── Bar-by-bar entry: vol_surge ──
            ...
            # ── Keep Raising entry (after 10:30, when no position) ──
            if kr_enabled and n_open < config.MAX_POSITIONS:
                bar_time = ts.time() if hasattr(ts, 'time') else None
                ...
                if bar_time and _dt.time(kr_start_h, kr_start_m) <= bar_time <= _dt.time(kr_end_h, kr_end_m):
                    # Scan every ~30 bars (roughly 30 seconds at 1-min bars)
                    # Find current bar index in timeline
                    cur_bar_global_idx = None
                    for gi, gts in enumerate(sorted_times):
                        if gts == ts:
                            cur_bar_global_idx = gi
                            break
                    if cur_bar_global_idx is not None and cur_bar_global_idx - kr_last_scan_bar_idx >= 30:
                        kr_last_scan_bar_idx = cur_bar_global_idx

                        # Score all cached symbols for keep-raising pattern
                        best_kr = None
                        best_score = 0
                        kr_lookback = getattr(config, "KR_LOOKBACK_MINUTES", 20)
                        kr_price_min = getattr(config, "KEEP_RAISING_PRICE_MIN", 1.0)
                        kr_price_max = getattr(config, "KEEP_RAISING_PRICE_MAX", 200.0)
                        ...
                        for sym, bars in cached_bars.items():
                            ...
                            for bi, b in enumerate(bars):
                                if b["timestamp"] == ts:
                                    cur_idx = bi
                                    break
                            if cur_idx is None or cur_idx < kr_lookback:
                                continue
                            # Get lookback window
                            window = bars[cur_idx - kr_lookback + 1:cur_idx + 1]
                            last_close = window[-1]["close"]
                            if not (kr_price_min <= last_close <= kr_price_max):
                                continue
                            score, metrics = score_keep_raising(window)
                            if score > best_score:
                                best_score = score
                                best_kr = (sym, cur_idx, last_close, score, metrics)

                        if best_kr:
                            sym, kr_bar_idx, kr_close, kr_score, kr_metrics = best_kr
                            # Enter keep_raising
                            current_equity = daily_start_equity + sum(p.pnl for p in closed_trades)
                            entry_price_actual = round(kr_close * (1 + entry_slippage), 4)
                            if config.MAX_POSITIONS <= 1:
                                pos_size = max(config.MIN_POSITION_SIZE, current_equity)
                            else:
                                pos_size = max(config.MIN_POSITION_SIZE, current_equity)
                            ...
                            if shares > 0:
                                ...
                                pos = OpenPosition(
                                    symbol=sym, shares=shares, entry_price=entry_price_actual,
                                    entry_bar_idx=kr_bar_idx, open_price=kr_close, rvol=0,
                                    atr=0, stop_pct=stop_p, target_pct=0.0,
                                    trail_activate_pct=trail_act_p, trail_pct=trail_p,
                                    signal_type="keep_raising",
                                )
                                open_positions.append(pos)
                                entered_symbols.add(sym)
                                n_open += 1
                                entry_ts_str = _bar_ts_str(cached_bars[sym], kr_bar_idx)
                                print(f"  {sym} [keep_raising] entry=${entry_price_actual:.4f}@{entry_ts_str} "
                                      f"{shares}sh [score={kr_score:.3f} up={kr_metrics['up_ratio']:.0%} "
                                      f"cons={kr_metrics['consistency']:.0%} amp={kr_metrics['amplitude_quality']:.0%}]")
    ...
    print(f"\n{'=' * 70}")
    print(f"[keep_raising_1.0] Backtest complete. Final equity: ${equity:,.2f}")
