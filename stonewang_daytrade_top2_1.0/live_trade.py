"""stonewang_daytrade_top2_1.0 — Top-2 Ranking + Open Entry + 5-Bar Add + Trailing Stop.

Strategy:
  - Pre-market scan for gap-up stocks, rank by RVOL → top 40
  - At 09:30: compute first5chg for all candidates, select top 2
  - Enter base position (30%) at open price for each selected stock
  - At 09:35: check 5-bar filter for each position:
      - PASS: Add 70% position, activate trailing stop (2% after +3% profit)
      - FAIL: Hold base with 5% stop, exit at 10:25
  - Exit: trailing stop, initial stop, or force close at 15:50

Backtest (3 months): +278.4%, 180 trades, 60.0% WR, R/R 4.8:1
"""
...
class Position:
    ...
    rvol: float = 0.0
    stop_pct: float = 0.03
    base_only: bool = False       # True if 5-bar filter failed (base position only)
    trailing_active: bool = False  # True once trailing stop is activated
    first5chg: float = 0.0
...
def _compute_first5chg(bars, open_price):
    """Compute first 5-minute change from bar accumulator data."""
    n_bars = getattr(config, "FRIK5BAR_BARS", 5)
    if len(bars) < n_bars:
        return 0.0
    bar5_close = bars[n_bars - 1]["close"]
    return (bar5_close / open_price) - 1.0
...
def run_trading_day(target_date):
    ...
    base_stop_pct = getattr(config, "BASE_STOP_PCT", 0.05)
    base_pct = getattr(config, "OPEN_ENTRY_BASE_PCT", 0.30)
    add_pct = getattr(config, "OPEN_ENTRY_ADD_PCT", 0.70)

    log(f"Starting trading day: {target_date.date()} (top-{top_n} by first5chg, trailing stop)")
    try:
    ...
    if not candidates:
        ...
        _wait_until(target_date, _parse_time(getattr(config, "FORCE_CLOSE_TIME", "15:50")))
        return
    ...
    entry_checked = set()
    entry_count = {}
    top2_selected = []    # Top-N selected candidates
    top2_ranked = False   # Whether we've done the ranking
    ...
    try:
        ...
        for ep in existing_positions:
            ...
            if sym in EXCLUDE_SYMBOLS:
                log(f"Skip {sym} — in EXCLUDE_SYMBOLS")
                continue
            ...
            pos = Position(
                ...
                signal_type=sp.get("signal_type", "top2"),
                highest=float(ep.current_price),
                rvol=rvol,
                stop_pct=sp.get("stop_pct", config.STOP_LOSS_PCT),
                base_only=sp.get("base_only", False),
                trailing_active=sp.get("trailing_active", False),
                first5chg=sp.get("first5chg", 0),
            )
    ...
    force_close_dt = dt.datetime.combine(target_date.date(),
                                          _parse_time(getattr(config, "FORCE_CLOSE_TIME", "15:50")),
                                          tzinfo=_EST)
    market_close_dt = dt.datetime.combine(target_date.date(), _parse_time(config.MARKET_CLOSE), tzinfo=_EST)
    mkt_open_dt = dt.datetime.combine(target_date.date(), dt.time(9, 30), tzinfo=_EST)
    filter_check_dt = dt.datetime.combine(target_date.date(), dt.time(9, 35), tzinfo=_EST)
    ...
    while True:
        ...
        if now >= force_close_dt:
            log("Force close time reached — closing all positions!")
            for pos in positions[:]:
                ...
                if sold > 0:
                    ...
                    trades_detail.append({"symbol": pos.symbol, "entry": pos.entry_price,
                                          "exit": round(fill, 4), "shares": sold,
                                          "pnl": round(pnl, 2), "reason": "force_close",
                                          "trade_type": pos.signal_type})
                    daily_trades += 1
                    positions.remove(pos)
                    log(f"FORCE CLOSE {pos.symbol}, P&L=${pnl:+,.2f}")
            break

        # Daily loss circuit breaker
        if max_daily_loss > 0 and daily_loss <= -max_daily_loss:
            log(f"Daily loss ${daily_loss:,.2f} exceeded limit ${max_daily_loss:,.2f}")
            for pos in positions[:]:
                sold, fill = force_sell_position(pos.symbol, pos.shares)
                if sold > 0:
                    pnl = (fill - pos.entry_price) * sold if fill > 0 else 0
                    trades_detail.append({"symbol": pos.symbol, "entry": pos.entry_price,
                                          "exit": fill, "shares": sold, "pnl": round(pnl, 2),
                                          "reason": "circuit_breaker", "trade_type": pos.signal_type})
                    daily_trades += 1
                    positions.remove(pos)
        ...
        # ── PHASE 1: Open entry at 09:30 ──
        # Rank candidates by first5chg once we have 5 bars (or at 09:35)
        if not top2_ranked and now >= filter_check_dt:
            log("Ranking candidates by first5chg...")
            scored = []
            for c in candidates:
                sym = c["symbol"]
                bars = _accumulator.get_1min_bars(sym)
                f5 = _compute_first5chg(bars, c["open_price"])
                scored.append({**c, "first5chg": f5})
            scored.sort(key=lambda x: x["first5chg"], reverse=True)
            top2_selected = scored[:top_n]
            top2_ranked = True

            log(f"Top-{top_n} selected:")
            for s in top2_selected:
                log(f"  {s['symbol']}: first5chg={s['first5chg']:+.2%} gap=+{s['gap_pct']:.1%} RVOL={s['rvol']:.1f}x")

            # Enter base positions for top-N
            try:
            ...
            for s in top2_selected:
                ...
                if any(p.symbol == sym for p in positions):
                    continue
                if sym in EXCLUDE_SYMBOLS or is_crypto_etf(sym):
                    continue
                ...
                base_slot = pos_size * base_pct
                base_shares = int(base_slot / entry_price_actual)
                if base_shares <= 0:
                    continue

                order, _, reject = place_buy_market(sym, base_shares)
                if order is None:
                    log(f"Base entry rejected: {sym} - {reject}")
                    continue
                ...
                pos = Position(
                    ...
                    gap_pct=s["gap_pct"], signal_type="top2_base",
                    highest=fill_price, rvol=rvol,
                    stop_pct=base_stop_pct, base_only=True,
                    first5chg=s["first5chg"],
                )
                positions.append(pos)
                entry_checked.add(sym)
                daily_trades += 1
                live_bp -= fill_price * filled
                log(f"BASE ENTRY {sym} {filled}sh @ ${fill_price:.4f} "
                    f"[first5chg={s['first5chg']:+.2%} RVOL={rvol:.1f}x stop={base_stop_pct:.0%}]")

        # ── PHASE 2: 5-bar filter check at 09:35 → add position ──
        if top2_ranked and now >= filter_check_dt:
            for pos in positions[:]:
                if not pos.base_only:
                    continue
                if pos.signal_type != "top2_base":
                    continue
                ...
                passed, reason = check_frik5bar_filter(bars, pos.open_price, pos.gap_pct)

                if passed:
                    # Add position at current price
                    add_price = bars[-1]["close"] if bars else pos.open_price
                    entry_slippage = getattr(config, "SLIPPAGE_ENTRY_PCT", 0.005)
                    add_entry = round(add_price * (1 + entry_slippage), 4)
                    pos_size = max(config.MIN_POSITION_SIZE, get_rvol_sizing(pos.rvol, equity))
                    add_slot = pos_size * add_pct
                    add_shares = int(add_slot / add_entry)
                    if add_shares <= 0:
                        # Just update stop and mark as not base_only
                        pos.base_only = False
                        pos.signal_type = "top2_added"
                        pos.stop_pct = config.STOP_LOSS_PCT
                        log(f"5-BAR PASS {pos.symbol} (no add shares) → trailing stop active")
                        continue

                    order, _, reject = place_buy_market(pos.symbol, add_shares)
                    if order is None:
                        log(f"Add entry rejected: {pos.symbol} - {reject}")
                        pos.base_only = False
                        pos.signal_type = "top2_added"
                        pos.stop_pct = config.STOP_LOSS_PCT
                        continue
                    ...
                    if filled <= 0:
                        ...
                        continue
                    if fill_price <= 0:
                        fill_price = add_entry

                    # Update position with weighted average entry
                    total_cost = pos.entry_price * pos.shares + fill_price * filled
                    total_shares = pos.shares + filled
                    ...
                    pos.stop_pct = config.STOP_LOSS_PCT
                    daily_trades += 1
                    log(f"ADD {pos.symbol} {filled}sh @ ${fill_price:.4f} → "
                        f"total {total_shares}sh avg=${pos.entry_price:.4f} [trailing stop active]")
                else:
                    # Filter failed — keep base only with wide stop, exit at 10:25
                    log(f"5-BAR FAIL {pos.symbol}: {reason} — holding base, exit at 10:25")
                    # Mark as checked so we don't re-check
                    pos.signal_type = "top2_base_nofilter"
        ...
        for pos in positions[:]:
            ...
            if bar_low <= stop_price:
                reason = "stop_loss"
            # 2. Trailing stop (for confirmed positions)
            elif not pos.base_only:
                if not pos.trailing_active and pos.highest / pos.entry_price - 1 >= trail_start:
                    pos.trailing_active = True
                if pos.trailing_active:
                    trail_stop = round(pos.highest * (1 - trail_pct), 4)
                    if bar_low <= trail_stop:
                        reason = "trail_stop"
            # 3. Time exit (for base-only positions when filter failed)
            elif pos.base_only and bar_time is not None and bar_time >= exit_time:
                reason = "10:25_exit"
            ...
            if sold <= 0:
                ...
                log(f"SELL stuck for {pos.symbol}, throttling 60s")
                continue
            if fill <= 0:
                fill = cur_price
        ...
        if ws_needs_restart and time.time() - _stream_state.get("last_restart_ts", 0) > 30:
            ...
            try:
                ...
                log(f"WebSocket restart failed: {e}")
        ...
        state.update({
            ...
            "positions": [{"symbol": p.symbol, "shares": p.shares, "entry_price": p.entry_price,
                           ...
                           "stop_pct": p.stop_pct, "highest": p.highest,
                           "base_only": p.base_only, "trailing_active": p.trailing_active,
                           "first5chg": p.first5chg} for p in positions],
...
def main():
    global _log_file
    _log_file = open(os.path.join(_ver_dir, "live_top2.log"), "a")

    top_n = getattr(config, "TOP_N", 2)
    log(f"Using {config.DATA_FEED.upper()} data feed")
    log("=" * 60)
    log(f"stonewang top2 1.0 — Top-{top_n} Ranking + Open Entry + Trailing Stop")
    log(f"Entry: open (30%) → 5-bar check → add (70%) if pass")
    log(f"Exit: trailing stop ({config.TRAIL_PCT:.0%} after +{config.TRAIL_START:.0%}) or "
        f"{config.STOP_LOSS_PCT:.0%} stop")
    log(f"Base fail: {config.BASE_STOP_PCT:.0%} stop / 10:25 exit")
    if getattr(config, "FRIK5BAR_ENABLED", False):
        ...
        log(f"5-Bar filter: {' + '.join(filter_parts)}")
    ...
    log(f"Force close: {getattr(config, 'FORCE_CLOSE_TIME', '15:50')}")
    log("=" * 60)
