"""Backtesting engine — stonewang_daytrade_1025out_2.0: RTG + RVOL-Adaptive Stop + 10:25 Exit.
...
Entry detection (1-min bars, approximates live WebSocket bar stream):
  Signal A (Red-to-Green):
    ...
    - bar[i].close >= open_price * (1 + RTG_MIN_BAR_GAIN_PCT) (>=1% gain)
...
Exit: 10:25 EST market sell or RVOL-adaptive stop loss.
"""
...
def _get_rvol_exit_tier(rvol):
    """Get adaptive stop/target based on RVOL tier.
    Returns (stop_pct, target_pct).
    """
...
def run_backtest(end_date=None, n_days=None):
    ...
    min_rvol_to_trade = getattr(config, "MIN_RVOL_TO_TRADE", 3.0)

    print(f"[1025out_2.0] Backtesting {len(trading_days)} trading days: "
    ...
    print(f"  Signal A (RTG): close > open AND close >= open*(1+{getattr(config, 'RTG_MIN_BAR_GAIN_PCT', 0.01):.0%}) "
          f"AND vol >= {config.RTG_VOLUME_MULT}× prior AND vol >= {config.RTG_MIN_VOLUME:,}")
    print(f"  Signal B (GapGo): DISABLED")
    print(f"  MIN_RVOL_TO_TRADE: {min_rvol_to_trade}× (filter low-momentum stocks)")
    ...
    if exit_tiers:
        print(f"  Exit tiers: " + ", ".join(f"RVOL>{r:.0f}×→{s:.0%}stop/{t:.0%}target" for r, s, t in exit_tiers))
    print(f"  Time exit: {getattr(config, 'EXIT_TIME', '10:25')} market sell")
    ...
    for date in trading_days:
        ...
        candidates = candidates[candidates["rvol"] >= min_rvol_to_trade]
        filtered_out = pre_filter - len(candidates)
        if filtered_out > 0:
            print(f"  [RVOL filter] Removed {filtered_out} candidates with RVOL < {min_rvol_to_trade}×")
        ...
        for _, row in candidates.iterrows():
            ...
            print(f"  {sym} gap={gap_pct:+.1%} RVOL={rvol:.1f}× open=${open_price:.2f} stop={stop_pct:.0%}")
        ...
        for _, row in candidates.iterrows():
            ...
            # RVOL-adaptive stop loss (key 2.0 improvement)
            stop_p, _ = _get_rvol_exit_tier(rvol)

            entry_slippage = getattr(config, "SLIPPAGE_ENTRY_PCT", 0.005)
            same_tier = tier_counts.get(_get_rvol_tier(rvol)[0], 1)
            exit_time = getattr(config, "EXIT_TIME", "10:25")
            ...
            while True:
                ...
                # 1025out exit: RVOL-adaptive stop loss or 10:25 time-based exit
                exit_price = 0.0
    ...
    print(f"\n{'=' * 70}")
    print(f"[1025out_2.0] Backtest complete. Final equity: ${equity:,.2f}")
    ...
    if all_trades:
        ...
        # RVOL tier breakdown
        rvol_buckets = {"high(>10x)": 0, "mid(5-10x)": 0, "low(<5x)": 0}
        rvol_pnl = {"high(>10x)": 0.0, "mid(5-10x)": 0.0, "low(<5x)": 0.0}
        for t in all_trades:
            # Find rvol from the original candidate data
            rvol_t = getattr(t, "rvol", 0)
            if rvol_t >= 10:
                bucket = "high(>10x)"
            elif rvol_t >= 5:
                bucket = "mid(5-10x)"
                ...
                bucket = "low(<5x)"
            rvol_buckets[bucket] += 1
            rvol_pnl[bucket] += t.pnl
        print(f"RVOL tier trades: {rvol_buckets}")
