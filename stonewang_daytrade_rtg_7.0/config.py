...
MAX_CANDIDATES = 5   # Scan top 5 by RVOL, but only trade #1 (full all-in)
...
PROGRESSIVE_TRAIL_TIERS = [
    (0.07, 0.000),  # profit > 7%  → trail = 0% (lock at high, exit on any pullback)
    (0.06, 0.002),  # profit > 6%  → trail = 0.2%
    (0.05, 0.004),  # profit > 5%  → trail = 0.4%
    (0.04, 0.006),  # profit > 4%  → trail = 0.6%
    (0.03, 0.008),  # profit > 3%  → trail = 0.8%
    (0.02, 0.010),  # profit > 2%  → trail = 1.0%
]

# ── Afternoon momentum scan ──────────────────────────────────────────
AFTERNOON_SCAN_ENABLED = True
AFTERNOON_PRICE_MAX = 200.0       # After 10:30, allow stocks up to $200 (not just small-cap)
...
DAILY_PROFIT_PROTECT_RATIO = 0.90    # Allow 10% drawdown from peak
DAILY_PROFIT_PROTECT_MIN = 5.0       # Activate when peak profit >= $5
DAILY_PROFIT_PROTECT_DELAY_SEC = 180   # 3 min delay after open (10 min too long — MYSZ lost $5.41 before 09:40 activation)

# ── Position sizing: FULL ALL-IN on single best candidate ────────────
RVOL_SIZING_TIERS = [
    (10.0, 1.00),   # A+: 100% equity (full all-in)
    (5.0,  1.00),   # A:  100% equity
    (2.0,  1.00),   # B:  100% equity
]
...
# ── Position sizing ──────────────────────────────────────────────────
INITIAL_CAPITAL = 281.78      # Account equity (updated 2026-09-14)
