"""stonewang_daytrade_rtg_6.0 — Opening Range Breakout + ATR Stops.
...
def get_atr_stop_params(rvol, atr, entry_price, gap_pct=0):
    ...
    target_pct = 0.0
    trail_activate_pct = 0.01  # Activate at 1% profit to reach all progressive tiers
...
def force_sell_position(symbol, shares):
    ...
    try:
        ...
        try:
            ...
            pass
        # Fallback: if fill_price still 0, use latest bar close
        if fill_price <= 0:
...
def _fetch_20d_avg_volumes(symbols, target_date):
    ...
    lookback = target_date - pd.Timedelta(days=45)
    avg_vols = {}
    atrs = {}
...
def scan_gaps(target_date):
    ...
    for _, row in results.iterrows():
        ...
        candidates.append({
            ...
            "rvol": float(rvol), "atr": float(atrs.get(sym, 0)),
        })
    # Filter by quality before selecting top candidates
    min_rvol = getattr(config, "MIN_ENTRY_RVOL", 2.0)
    min_price = getattr(config, "MIN_ENTRY_PRICE", 2.0)
    candidates = [c for c in candidates if c["rvol"] >= min_rvol and c["open_price"] >= min_price]
    candidates.sort(key=lambda c: c["rvol"], reverse=True)
    candidates = candidates[:config.MAX_CANDIDATES]
    log(f"Top {len(candidates)} candidates by RVOL (≥{min_rvol:.0f}×, ≥${min_price:.0f}): {[c['symbol'] for c in candidates]}")
    for c in candidates:
        atr_str = f", ATR=${c['atr']:.4f}" if c["atr"] > 0 else ""
...
def scan_afternoon_momentum(target_date):
    ...
    # Fetch daily bars (last 5 days) to get today's price + previous close
    today_data = {}
    batch_size = 200
...
def check_orb_entry(symbol, open_price, bars, min_volume=None, after_time=None):
    ...
    for i in range(orb_bars, len(market_bars)):
        ...
        bv = bar["volume"]
        pv = prev["volume"]
        if bc > range_high and pv > 0 and bv >= config.RTG_VOLUME_MULT * pv and bv >= min_volume:
            if getattr(config, "RTG_ENTRY_AT_OPEN", True):
                entry = round(open_price * 1.001, 4)
            else:
...
def check_momentum_entry(symbol, bars, min_volume=None):
    ...
    Criteria:
    - Last 5 bars: at least 3 up bars (close > open)
    - Volume trend: increasing over recent bars
    - Breakout: current close > recent high
    - Volume confirmation: current volume > recent avg × 1.5
    """
    if len(bars) < 6:
        return 0.0, False, ""
    if min_volume is None:
...
def run_trading_day(target_date):
    ...
    _last_afternoon_scan = 0  # timestamp of last afternoon scan
    ...
    try:
        ...
        for ep in existing_positions:
            ...
            atr_val = sp.get("atr", cand.get("atr", 0) if cand else 0)
            entry_px = float(ep.avg_entry_price)
            gap_pct = sp.get("gap_pct", cand.get("gap_pct", 0) if cand else 0)
            # ATR-based stops when available, else RVOL fallback
            if atr_val > 0:
                stop_p, target_p, trail_act_p, trail_p = get_atr_stop_params(
                    rvol, atr_val, entry_px, gap_pct=gap_pct)
            else:
    ...
    daily_trades = 0
    trades_detail = []
    # Restore trades from previous state (survive restart within same day)
    if prev_state_for_restart.get("trades_detail"):
        prev_date = prev_state_for_restart.get("date", "")
        if prev_date == str(target_date.date()):
            trades_detail = list(prev_state_for_restart["trades_detail"])
            daily_trades = len(trades_detail)
            daily_loss = sum(t.get("pnl", 0) for t in trades_detail)
            log(f"Restored {len(trades_detail)} trades from previous state (P&L=${daily_loss:+,.2f})")
    daily_loss = 0.0 if not trades_detail else daily_loss
    max_daily_loss = equity * config.MAX_DAILY_LOSS_PCT
    max_daily_profit = prev_state_for_restart.get("max_daily_profit", 0.0) if prev_state_for_restart.get("date") == str(target_date.date()) else 0.0
    # Ensure max_daily_profit >= current daily_loss (already realized profits are the floor)
    if daily_loss > max_daily_profit:
        max_daily_profit = daily_loss
        log(f"max_daily_profit adjusted to daily_loss: ${max_daily_profit:+,.2f}")
    ...
    while True:
            ...
            if (now - mkt_open_ts).total_seconds() >= protect_delay:
                ...
                if max_daily_profit >= protect_min and current_profit < max_daily_profit * protect_ratio:
                    ...
                    for pos in positions[:]:
                        ...
                        # Check price direction: compare current close to previous bar close
                        # Price declining → close; price rising/flat → keep
        ...
        for pos in positions[:]:
            ...
            if bar_low <= stop_price:
                ...
                if pos.trail_active:
                    # Progressive trailing: tighten trail as profit grows
                ...
                if reason is None and pos.target_pct > 0:
                    ...
                    if bar_high >= target_price:
                        reason = "target"
                if reason is None and config.RTG_TIME_LIMIT_SEC > 0 and time.time() - pos.entry_ts >= config.RTG_TIME_LIMIT_SEC:
                    reason = "time_limit"
            ...
            if sold <= 0:
                ...
                time.sleep(2)
                sold, fill = place_sell_market(pos.symbol, pos.shares)
            # If still failed, check if position still exists at Alpaca
            if sold <= 0:
                pos_exists = False
                try:
                    trading_client.get_open_position(pos.symbol)
                    pos_exists = True
                except Exception:
                    pass  # Position not found at Alpaca
                if pos_exists:
                    # Position exists but sell stuck (locked shares) — throttle retry
                    _sell_stuck_until[pos.symbol] = time.time() + 60
                    log(f"SELL stuck for {pos.symbol} (locked shares), throttling 60s")
                    continue
                else:
                    # Position gone at Alpaca — desync, treat as already closed
                    log(f"Position {pos.symbol} not found at Alpaca, removing from tracker (desync)")
                    positions.remove(pos)
                    _last_exit_ts[pos.symbol] = time.time()
                    entry_checked.discard(pos.symbol)
                    continue
            # Successful sell — record P&L
            if fill <= 0:
                fill = cur_price
        ...
        if entry_start_dt <= now < entry_end_dt and len(positions) < config.MAX_POSITIONS:
            ...
            for c in candidates:
                ...
                if (force_close_dt - now).total_seconds() < no_entry_before_close_min * 60:
                    continue
                ...
                if filled <= 0:
                    # Order not filled — verify it wasn't rejected before giving up
                    try:
                        order_check = trading_client.get_order_by_id(str(order.id))
                        if order_check.status in (OrderStatus.CANCELED, OrderStatus.REJECTED, OrderStatus.EXPIRED):
                            log(f"BUY {sym} order {order_check.status.name}, skipping")
                    except Exception:
                        pass
                    entry_checked.add(sym)
                    continue
                # Verify position exists at Alpaca (defend against desync)
                try:
                    alpaca_pos = trading_client.get_open_position(sym)
                    actual_qty = int(float(alpaca_pos.qty))
                    if actual_qty != filled:
                        log(f"Position desync: {sym} Alpaca={actual_qty}sh vs filled={filled}sh, using Alpaca")
                        filled = actual_qty
                        fill_price = float(alpaca_pos.avg_entry_price)
                except Exception:
                    log(f"Position {sym} not found at Alpaca after buy fill, skipping (desync)")
                    entry_checked.add(sym)
                    continue
                if fill_price <= 0:
                ...
                if atr_val > 0:
                    stop_p, target_p, trail_act_p, trail_p = get_atr_stop_params(
                        rvol, atr_val, fill_price, gap_pct=c["gap_pct"])
                else:
                    stop_p, target_p, trail_act_p, trail_p = get_rvol_exit_params(rvol)
        ...
        # WS health — REST backfill is primary data source (every 10s)
        # WS is supplemental; only restart if stale for 5 min (data truly missing)
        _ws_restart_count = _stream_state.get("restart_count", 0)
        ws_stale = time.time() - _stream_state["last_bar_ts"] > 300
        ws_needs_restart = ws_stale  # Don't restart just because WS disconnected
        # Exponential backoff: 60s → 120s → 240s → 480s → 600s (max)
        _ws_backoff = min(60 * (2 ** min(_ws_restart_count, 4)), 600)
        ...
        # REST backfill: always poll every 10s to keep accumulator fresh
        # This is the primary data source when WS is down (common for low-liquidity stocks)
        if time.time() - _stream_state.get("last_backfill_ts", 0) > 10:
            try:
                backfill_1min_bars(syms, target_date)
                _stream_state["last_backfill_ts"] = time.time()
                # Also update last_bar_ts so WS doesn't get falsely restarted
                _stream_state["last_bar_ts"] = time.time()
            except Exception as e:
